...
LUA_SHA1 = get_hash(os.path.join(CWD, 'priimage.lua'))
...
class WgetArgs(object):
    ...
    def realize(self, item):
        wget_args = [
            ...
            '--connect-timeout', '1',
            '--tries', 'inf',
            '--domains', 'prcm.jp,prepics.com,prepics-cdn.com',
            '--span-hosts',
            '--waitretry', '30',
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'pic':
                if ':' not in item_value:
                    continue
                album_id, pic_id = item_value.split(':', 1)
                wget_args.extend(['--warc-header', 'priimage-pic-id: '+pic_id])
                wget_args.append('https://prcm.jp/album/{}/pic/{}'.format(album_id, pic_id))
            elif item_type == 'album':
                wget_args.extend(['--warc-header', 'priimage-album-id: '+item_value])
                wget_args.append('https://prcm.jp/album/'+item_value)
            elif item_type == 'user':
                wget_args.extend(['--warc-header', 'priimage-user-id: '+item_value])
                wget_args.append('https://prcm.jp/user/'+item_value)
            elif item_type == 'list':
                wget_args.extend(['--warc-header', 'priimage-list: '+item_value])
                wget_args.append('https://prcm.jp/list/'+item_value)
            elif item_type == 'talk':
                wget_args.extend(['--warc-header', 'priimage-talk-id: '+item_value])
                wget_args.append('https://prcm.jp/talk/'+item_value)
            elif item_type == 'talk-list':
                wget_args.extend(['--warc-header', 'priimage-talk-list: '+item_value])
                wget_args.append('https://prcm.jp/talk/list/'+item_value)
            elif item_type == 'article':
                wget_args.extend(['--warc-header', 'priimage-article-id: '+item_value])
                wget_args.append('https://news.prcm.jp/article/'+item_value)
            elif item_type == 'cdn':
                wget_args.extend(['--warc-header', 'cdn-url: '+item_value])
...
project = Project(
    title=TRACKER_ID,
    project_html='''
        <img class="project-logo" alt="Project logo" src="https://wiki.archiveteam.org/images/c/c9/Pri-image-icon.png" height="50px" title=""/>
        <h2>プリ画像 <span class="links"><a href="https://prcm.jp/">Website</a> &middot; <a href="http://tracker.archiveteam.org/priimage/">Leaderboard</a> &middot; <a href="https://wiki.archiveteam.org/index.php/プリ画像">Wiki</a></span></h2>
        <p>Archiving プリ画像.</p>
    '''
)
